...
const allDefs = Object.entries(definitions).filter(notTraversable).map(([ fn, spec ]) => {
  const {
    args,
    extending = [],
  ...
  function getParameterType (arg: string) {
    if (arg.indexOf(name) !== -1) return arg.replace(new RegExp(`${name}<\(\.*\)>`), 'Kind<Type, $1>')
